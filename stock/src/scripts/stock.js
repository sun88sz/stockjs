...
var K = /** @class */ (function () {
    function K(open, close, high, low, timeBegin, volume, amount) {
        this.open = open;
        this.close = close;
        this.high = high;
        this.low = low;
...
// ]);
function loadDATA() {
    var ajax = new Ajax('http://127.0.0.1:8080/stock', { 'Content-Type': 'text/plain' });
...
function setData(rawData) {
    ...
    for (var _i = 0, rawData_1 = rawData; _i < rawData_1.length; _i++) {
        ...
        //数据数组，即数组中除日期外的数据
        // alert(rawData[i]);
        var x = [k.open, k.close, k.high, k.low];
        values.push(x);
    }
